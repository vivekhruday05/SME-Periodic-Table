...
class AppState(TypedDict):
    ...
    # Primary (legacy) single task for backward compatibility
    task_type: Optional[Literal["quiz", "report", "presentation", "knowledge"]]
    # NEW: Multiple tasks extracted from the prompt (e.g. ["knowledge", "quiz"]) 
    tasks: Optional[List[Literal["quiz", "report", "presentation", "knowledge", "explanation"]]]
    ...
    # Content to be PDF'd (merged if multiple content pieces)
...
# -------------------------------------------------------------------
# Node 1 — Extract Task Details
# -------------------------------------------------------------------
def llm_extractor(state: AppState):
    logger.info("🧠 Step 1: Extracting key details from prompt (multi-task aware)...")

    extraction_prompt = f"""
    You are a task detail extractor. Extract key fields and ALL tasks requested.
    ...
    Rules:
    - Include "knowledge" if user asks a general question ("tell me about", "what is", "explain", "describe").
    - Include both "knowledge" (or "explanation") AND "quiz" if they ask for an explanation and a quiz.
    - If both "report" and "presentation" are requested, include both (presentation implies report creation first).
    - Set task_type to the FIRST requested generative artifact among quiz/report/presentation if present; otherwise knowledge.
...
# -------------------------------------------------------------------
# Node 3c — Explanation Generation (MODIFIED)
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
# Node 3d — Presentation Generation (Unmodified)
# -------------------------------------------------------------------
def generate_presentation(state: AppState):
    logger.info("🖥️ Step 3d: Generating Presentation...")
    try:
...
# -------------------------------------------------------------------
# Node 4 — PDF Generation (Unmodified)
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
# NEW: Node 6 — Loop Control: Pop Task
# -------------------------------------------------------------------
...
def route_tasks(state: AppState):
    ...
    if next_task == "presentation":
        ...
        if not state.get("report_content"):
            logger.warning("Task 'presentation' needs 'report' first. Re-routing to 'generate_report'.")
            # This logic means 'report' will run, then 'pop_task' will remove 'presentation'.
            # A more robust graph would insert 'report' before 'presentation' in the extractor.
            # For now, we run 'report' *instead* of presentation if report not ready.
            # A better way: check if 'report' is ALSO in the tasks list.
            if "report" not in state.get("tasks", []):
                # If report isn't in the list, we MUST run it now
                return "generate_report"
            else:
                # Report is already in the list, it will run first. Skip for now.
                logger.info("Skipping 'presentation' for now, 'report' will run first.")
                return "pop_task" # Skip presentation
        else:
...
# -------------------------------------------------------------------
# Build LangGraph Workflow (NEW LOOP)
# -------------------------------------------------------------------
...
workflow.add_node("pop_task", pop_task) # The node that advances the loop
...
workflow.add_conditional_edges(
    ...
    route_tasks,
    {
        ...
        "generate_pdf": "generate_pdf", # If no tasks, just make PDF
        "pop_task": "pop_task" # Handle unknown/skip
    }
)
...
workflow.add_conditional_edges(
    ...
    route_tasks, # The dispatcher routes again
    {
        ...
        "generate_quiz": "generate_quiz",
        "generate_presentation": "generate_presentation",
        "generate_pdf": "generate_pdf", # This branch is taken when tasks are empty
        "pop_task": "pop_task" # Handle unknown/skip
    }
)
# -------------------------
